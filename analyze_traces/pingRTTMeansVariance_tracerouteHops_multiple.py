...
def generate_report(mes_ids, probe_ids, command, name):
    ...
    if command == "PING":
        report_name = os.path.join(ANALYZED_TRACE_FILE, "{0}_{1}_report.csv".format(command, name))
        ...
        with open(report_name, 'wb') as f_handler:
            ...
            a.writerow(
                [
                    ...
                    'variace from LISP-Lab', 'variace from mPlane', 'variace from FranceIX', 'variace from rmd']
            )
    ...
    if command == "TRACEROUTE":
        ...
        with open(report_name, 'wb') as f_handler:
            ...
            a.writerow(['mesurement id', 'target of traceroute',
                        'hops number from LISP-Lab', 'hops number from mPlane', 'hops number from FranceIX', 'hops number from rmd'])
            for mes_id in mes_ids:
                output_row = [mes_id]
